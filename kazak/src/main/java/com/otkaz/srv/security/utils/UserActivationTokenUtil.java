...
@Component
public class UserActivationTokenUtil implements Serializable {
	...
	public String generateToken(UserDto user) {
		...
		try {
			...
			jedis.setex(getRedisKey(token), Constants.USER_ACTIVATION_TOKEN_VALIDITY_SECONDS, user.getEmail());
			return token;
		} finally {
			if (jedis != null) {
				jedis.close();
	...
	private String getRedisKey(String token) {
		return instanceKey + "-oft-ua-token-" + token;
	}
}
